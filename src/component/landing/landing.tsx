  ...
  return (
    <main className="xp-login-desktop">
      <section className="xp-login-window" aria-labelledby="login-title">
        ...
        <div className="xp-login-interior">
          <aside className="xp-login-sidebar"><XPIcon name="users" size={48} /><p>A Better<br />Student<br />Tomorrow.</p><span>— 1337</span></aside>
        <div className="xp-login-body">
          <div className="xp-login-brand"><XPIcon name="computer" size={100} /><div><h1 id="login-title">1337LEETS</h1><p>ELITE SCHOOL RANKING</p></div></div>
          <hr />
          <h2>Welcome back</h2>
          <p className="xp-login-description">Your progress, peers, and tools. All in one place.</p>
          <p className="xp-login-note"><span className="xp-login-info" aria-hidden="true">i</span><span>Many of you said the old UI felt like too much, so I gave it a simpler, Windows XP-inspired look.</span></p>
          <button className="xp-login-button" disabled={isLoggingIn} onClick={() => { setIsLoggingIn(true); window.location.href = process.env.NEXT_PUBLIC_REDIRECT_URL as string; }}>
            <XPIcon name="users" size={34} />{isLoggingIn ? "Signing in…" : "Login to Platform"}
          </button>
